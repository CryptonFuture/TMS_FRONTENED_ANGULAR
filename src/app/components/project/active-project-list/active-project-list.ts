...
@Component({
  selector: 'app-active-project-list',
  ...
  templateUrl: './active-project-list.html',
  styleUrl: './active-project-list.scss'
})
export class ActiveProjectList implements AfterViewInit, OnDestroy, OnInit {
   ...
   displayedColumns: string[] = [
    ...
    'is_allow_off_time',
    'action'];
  activeProject: any = new MatTableDataSource([]);
  ...
  ngAfterViewInit(): void {
    this.activeProject.paginator = this.paginator;
  }
  ...
  getActiveProject(): void {
    forkJoin([
      this._projServices.getActiveProject().pipe(retry(3), catchError(err => of(null))),
      this._projServices.activeCountProject().pipe(retry(3), catchError(err => of(null)))
    ]).pipe(takeUntil(this.destroy$)).subscribe(([
      project,
      activeProj
    ]) => {
      this.activeProject = project,
      this.projectActiveCount = activeProj.count

      console.log(this.activeProject, 'task');
  ...
  ngOnDestroy(): void {
   ...
   toggleDeleted(id: string): void {
        const element = this.activeProject.find((item: any) => item._id === id);
