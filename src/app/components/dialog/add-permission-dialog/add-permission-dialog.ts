...
import { createPermissionForm } from '../../auth-forms/permission/permission.form';
...
export class AddPermissionDialog implements OnInit, OnDestroy {
  ...
  onCheckboxChange(event: any, action: string) {
    const actionsArray = this.addPermissionForm.get('action') as FormArray;
 ...
 onSubmit(): void {
      ...
      this.addPermissionForm.disable()
      this._permissionService.addPermission(this.addPermissionForm.value).subscribe({
        next: (response) => {
          this.addPermissionForm.enable()
           if(response.success) {
              ...
              Object.keys(this.addPermissionForm.controls).forEach(key => {
                this.addPermissionForm.get(key)?.setErrors(null);
              });
              setTimeout(() => {
