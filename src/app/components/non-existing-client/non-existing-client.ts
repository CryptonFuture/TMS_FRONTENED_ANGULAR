...
@Component({
  selector: 'app-non-existing-client',
  ...
  templateUrl: './non-existing-client.html',
  styleUrl: './non-existing-client.scss'
})
export class NonExistingClient implements AfterViewInit, OnDestroy, OnInit {
   ...
   displayedColumns: string[] = ['serialNo', 'name', 'email', 'description', 'start_time', 'end_time', 'status', 'is_deleted', 'action'];
    ...
    getClientCount(): void {
      forkJoin([
        ...
        this.NonExistClient = nonExistingClient
        this.NonExistCount = countNonExisting.count
        console.log(this.NonExistClient, 'exist');
        console.log(this.NonExistCount, 'count');
    ...
    onEditClientDialog(data: any): void {
      ...
      const dialogRef = this._matdialog.open(ConfirmDialog, {
        width: '300px',
        data: { message: 'Are you sure you want to delete this non existing client?' }
      });
   ...
   toggleDeleted(id: string): void {
        const element = this.NonExistClient.find((item: any) => item._id === id);
