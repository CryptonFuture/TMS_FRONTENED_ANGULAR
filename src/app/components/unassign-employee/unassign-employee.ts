...
export class UnassignEmployee implements OnDestroy, OnInit, AfterViewInit {
  ...
  unAssignEmpToClient: any = new MatTableDataSource([]);
  unAssignEmpToClientCount: any
  ...
  ngAfterViewInit(): void {
     this.unAssignEmpToClient.paginator = this.paginator;
  }

  ngOnInit(): void {
    this.getUnAssignCountEmp()
  }

  getUnAssignCountEmp(): void {
    forkJoin([
      this._empAllocService.getUnAssignToClientEmp().pipe(retry(3), catchError(err => of(null))),
      this._empAllocService.unAssignEmpToClientCount().pipe(retry(3), catchError(err => of(null)))

    ]).pipe(takeUntil(this.destroy$)).subscribe(([
      unAassignToClientEmployee,
      unAssignCountEmployee
    ]) => {
  ...
  ngOnDestroy(): void {
    ...
    onDelete(id: string): void {
      const dialogRef = this._matdialog.open(ConfirmDialog, {
        width: '300px',
        data: {message: 'Are you sure you want to delete this unAssign emp?'}
      });
      ...
      dialogRef.afterClosed().subscribe((confirmed: boolean) => {
       ...
       toggleDeleted(id: string): void {
        const element = this.unAssignEmpToClient.find((item: any) => item._id === id);
